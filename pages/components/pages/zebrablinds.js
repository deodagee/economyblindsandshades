...
function ZebraBlinds({ onUpdateTitle, onUpdateAttachment, sensitiveData }) {
  ...
  useEffect(() => {
    const storedProductTitle = LocalStorageAPI.getValue('Product_Title');
    const storedProductLightening = LocalStorageAPI.getValue('Product_Lightening');

    setProductTitle(storedProductTitle || '');
